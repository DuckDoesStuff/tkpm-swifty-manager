...
export default function SignUpForm() {
  ...
  const validateFields = (credential: Credential) => {
    ...
    // Check for unmatched confirm password
  ...
  const handleOnSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    message.loading({content: "Signing up...", key: "signin", duration: 0})
    ...
    try {
      ...
      if (!user) {
        message.error({content: "Failed to sign up", key: "signin"});
        return;
      }
      ...
      if (!response.ok) {
        message.error({content: "Failed to create merchant", key: "signin"});
        return;
      }
      ...
      signInWithEmailAndPassword(auth, credential.email, credential.password)
        .catch((error) => {
          console.log("Failed to sign in after signed up");
        });
      ...
      message.success({content: "Signed up successfully", key: "signin"});
      router.push("/");
    } catch (error: any) {
