...
export default function SignInForm() {
  ...
  const validateFields = (credential: Credential) => {
  ...
  const handleSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    message.loading({content: "Signing in...", key: "signin", duration: 0});
    const validation = validateFields(credential);

    if (!validation.isValid) {
      message.error({content: validation.message, key: "signin"});
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        message.error({content: "No user found in the database with that email", key: "signin"});
        return;
      }

      // we call firebase to signing
      ...
      if (userCredential.user == null) {
        message.error({content: "Error occurred", key: "signin"});
        return;
      }
      ...
      document.cookie = `swifty_merchant_session=${sessionData.sessionId}; path=/; Secure; SameSite=Strict`;

      // Successful sign in
      message.success({content: "Signed in", key: "signin"});
      router.push("/");
    } catch (error: any) {
      console.log(error);

      if (error.code === "auth/invalid-credential") {
        // setAlertMessage("Wrong password or email entered. Please try again.");
        message.error({content: "Wrong password or email", key: "signin"});
      } else {
        message.error({content: "Error occurred", key: "signin"});
      }
    }
  ...
  const handleSignUpWithGoogle = async () => {
    ...
    const response = await fetch(
      process.env.NEXT_PUBLIC_BACKEND_HOST + "/merchant/" + user.email,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
