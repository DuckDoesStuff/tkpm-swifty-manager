...
import {SlArrowDown, SlArrowUp, SlLogout} from "react-icons/sl";
        ...
        <nav className="mt-5 px-4 py-4 lg:mt-9 lg:px-6">
          ...
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
              ...
              {/* Menu Item Shop */}
              <SidebarLinkGroup activeCondition={pathname === "/shop"}>
                {(handleClick, open) => {
                  return (
                    <React.Fragment>
                      <Link
                        href="#"
                        className={`group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 
                          ${(pathname === "/shop" || pathname.includes("/shop")) &&
                        "bg-graydark dark:bg-meta-4"}`}
                        onClick={(e) => {
                          e.preventDefault();
                          sidebarExpanded ? handleClick() : setSidebarExpanded(true);
                        ...
                        <ul className="mb-5.5 mt-4 flex flex-col gap-2.5 pl-6">
                          ...
                          <li>
                            <Link
                              href="/shop/create"
                              className={`group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                pathname === "/shop/create" && "text-white"
                              }`}
                            >
                              Create a shop
                            </Link>
          ...
          <button onClick={handleLogout}
                  className={"text-white font-bold text-xl flex gap-5 items-center hover:text-primary"}>
            <SlLogout/>Log out
          </button>
        </nav>
        {/* <!-- Sidebar Menu --> */}
